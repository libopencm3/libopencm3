...
/* note: the rest of the file is taken from f4/rtc.c */
/* EXTI22 changes to EXTI17, NVIC_RTC_WKUP_IRQ changes to NVIC_RTC_IRQ */
/*---------------------------------------------------------------------------*/
...
void rtc_enable_wakeup_timer_interrupt(void)
{
	 ...
	 * required:
	 * 1. Configure and enable the EXTI Line 17 in interrupt mode and
	 *    select the rising edge sensitivity.
	 */
	exti_enable_request(EXTI17);
	/* trigger setting not needed, see stm32l562ce RM table 108 */
...
{
	/* 1. Disable EXTI Line 17 */
	exti_disable_request(EXTI17);

	/* 2. Disable RTC IRQ channel in the NVIC. */
