...
void usart_set_baudrate(uint32_t usart, uint32_t baud)
{
	...
	if (usart != LPUART1) {
		baudrate = ((2 * clock) + baud) / (2 * baud);
	} else {
		baudrate = (256 * clock)/baud;
	}
	

	/* TODO check oversampling 16 */
	USART_BRR(usart) = baudrate;
}
...
{
	if (bits == 7) {
		USART_CR1(usart) &= ~USART_CR1_M0; /* 7 data bits */
		USART_CR1(usart) |= USART_CR1_M1; 
	} else if (bits == 8) {
		USART_CR1(usart) &= ~USART_CR1_M0; /* 8 data bits */
		USART_CR1(usart) &= ~USART_CR1_M1; 
	} else {
		USART_CR1(usart) |= USART_CR1_M0; /* 9 data bits */
		USART_CR1(usart) &= ~USART_CR1_M1; 
	}
}
...
uint8_t usart_is_recv_ready(uint32_t usart)
{
