...
void ltdc_setup_windowing(uint8_t layer_number, uint16_t h_back_porch,
    ...
    LTDC_LxWHPCR(layer_number) =
        (h_back_porch + width + h_sync - 1) << LTDC_LxWHPCR_WHSPPOS_SHIFT |
        (h_sync + h_back_porch) << LTDC_LxWHPCR_WHSTPOS_SHIFT;
    LTDC_LxWVPCR(layer_number) =
        (v_back_porch + height + v_sync - 1) << LTDC_LxWVPCR_WVSPPOS_SHIFT |
