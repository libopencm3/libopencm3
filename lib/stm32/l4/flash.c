...
void flash_erase_all_sectors(void)
{
	...
	FLASH_CR |= FLASH_CR_MER1 | FLASH_CR_MER2; /* Enable mass erase. */
	FLASH_CR |= FLASH_CR_START;	/* Trigger the erase. */

	flash_wait_for_last_operation();
	FLASH_CR &= ~FLASH_CR_MER1 & ~FLASH_CR_MER2; /* Disable mass erase. */
}
...
void flash_program_option_bytes(uint32_t data)
{
	...
	FLASH_OPTR = data & ~0x3;
	FLASH_OPTR |= FLASH_CR_OPTSTRT;  /* Enable option byte prog. */
	flash_wait_for_last_operation();
}
/**@}*/
