...
{
	DMA_CTL(dma, channel) |= DMA_CTL_MEM2MEM;
	DMA_CTL(dma, channel) &= ~DMA_CTL_CMEN;
}
...
{
	DMA_CTL(dma, channel) &= ~(DMA_CTL_PRIO_MASK);
	DMA_CTL(dma, channel) |= prio;
}
...
void dma_set_memory_size(uint32_t dma, uint8_t channel, uint32_t mem_size)
{
	...
	DMA_CTL(dma, channel) &= ~(DMA_CTL_MWIDTH_MASK);
	DMA_CTL(dma, channel) |= mem_size;
}
...
{
	DMA_CTL(dma, channel) &= ~(DMA_CTL_PWIDTH_MASK);
	DMA_CTL(dma, channel) |= peripheral_size;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_MNAGA;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_PNAGA;
}
...
{
	DMA_CTL(dma, channel) &= ~DMA_CTL_DIR;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_ERRIE;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_HTFIE;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_FTFIE;
}
...
{
	DMA_CTL(dma, channel) |= DMA_CTL_CHEN;
}
...
{
	if (!(DMA_CTL(dma, channel) & DMA_CTL_CHEN)) {
		DMA_PADDR(dma, channel) = (uint32_t) address;
	}
}
...
{
	if (!(DMA_CTL(dma, channel) & DMA_CTL_CHEN)) {
		DMA_MADDR(dma, channel) = (uint32_t) address;
	}
}
