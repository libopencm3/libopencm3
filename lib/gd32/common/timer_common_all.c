...
@code
	...
	timer_set_oc_value(TIM1, TIM_OC0, 200);
...
@code
	...
	timer_ic_set_input(TIM3, TIM_IC0, TIM_IC_IN_TI1);
...
{
	TIM_DMAINTEN(timer_peripheral) |= irq;
}
...
Returns true if the specified interrupt flag (UIF, TIF or CCxIF, with BRKIF or
...
/* flag not set or interrupt disabled or not an interrupt source */
	if (((TIM_INTF(timer_peripheral) &
		TIM_DMAINTEN(timer_peripheral) & flag) == 0) ||
		(flag > TIM_INTF_BRKIF)) {
		return false;
	}
	return true;
...
{
	if ((TIM_INTF(timer_peripheral) & flag) != 0) {
		return true;
	}
...
void timer_clear_flag(uint32_t timer_peripheral, uint32_t flag)
{
	...
	TIM_INTF(timer_peripheral) = ~flag;
}
...
{
	TIM_CTL0(timer_peripheral) |= TIM_CTL0_ARSE;
}
...
{
	alignment &= TIM_CTL0_CAM_MASK;
	TIM_CTL0(timer_peripheral) &= ~TIM_CTL0_CAM_MASK;
	TIM_CTL0(timer_peripheral) |= alignment;
}
...
{
	TIM_CTL0(timer_peripheral) &= ~TIM_CTL0_DIR_DOWN;
}
...
{
	TIM_CTL0(timer_peripheral) |= TIM_CTL0_SPM;
}
...
{
	TIM_CTL0(timer_peripheral) &= ~TIM_CTL0_UPS;
}
...
{
	TIM_CTL0(timer_peripheral) &= ~TIM_CTL0_UPDIS;
}
...
{
	TIM_CTL0(timer_peripheral) |= TIM_CTL0_CEN;
}
...
{
	TIM_CTL1(timer_peripheral) |= outputs & TIM_CTL1_ISO_MASK;
}
...
{
	TIM_CTL1(timer_peripheral) |= TIM_CTL1_TI0S;
}
...
{
	TIM_CTL1(timer_peripheral) &= ~TIM_CTL1_MMC_MASK;
	TIM_CTL1(timer_peripheral) |= mode;
}
...
{
	TIM_CTL1(timer_peripheral) &= ~TIM_CTL1_DMAS;
}
...
shadowed, they are updated by software generating the COMG event (@ref
...
{
	TIM_CTL1(timer_peripheral) |= TIM_CTL1_CCUC;
}
...
/*---------------------------------------------------------------------------*/
/** @brief Enable Timer Capture/Compare Control shadow.

The capture/compare control bits CCxE, CCxNE and OCxM are set to be shadowed
...
{
	TIM_CTL1(timer_peripheral) |= TIM_CTL1_CCSE;
}

/*---------------------------------------------------------------------------*/
/** @brief Disable Timer Capture/Compare Control shadow.

The capture/compare control bits CCxE, CCxNE and OCxM shadow is disabled.
...
{
	TIM_CREP(timer_peripheral) = value;
}
...
{
	TIM_CAR(timer_peripheral) = period;
}
...
void timer_enable_oc_clear(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCEN;
		break;
	case TIM_OC1:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCEN;
		break;
	case TIM_OC2:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCEN;
		break;
	case TIM_OC3:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCEN;
		break;
	case TIM_OC0N:
	case TIM_OC1N:
	case TIM_OC2N:
...
void timer_set_oc_fast_mode(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMFEN;
		break;
	case TIM_OC1:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMFEN;
		break;
	case TIM_OC2:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMFEN;
		break;
	case TIM_OC3:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMFEN;
		break;
	case TIM_OC0N:
	case TIM_OC1N:
	case TIM_OC2N:
		       ...
		       enum tim_oc_mode oc_mode)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH0MS_MASK;
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0MS_OUT;
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH0COMCTL_MASK;
		switch (oc_mode) {
		case TIM_COMCTL_FROZEN:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_FROZEN;
			break;
		case TIM_COMCTL_ACTIVE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_ACTIVE;
			break;
		case TIM_COMCTL_INACTIVE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_INACTIVE;
			break;
		case TIM_COMCTL_TOGGLE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_TOGGLE;
			break;
		case TIM_COMCTL_FORCE_LOW:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_FORCE_LOW;
			break;
		case TIM_COMCTL_FORCE_HIGH:
			TIM_CHCTL0(timer_peripheral) |=
			    TIM_CHCTL0_CH0COMCTL_FORCE_HIGH;
			break;
		case TIM_COMCTL_PWM0:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_PWM0;
			break;
		case TIM_COMCTL_PWM1:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMCTL_PWM1;
			break;
		}
		break;
	case TIM_OC1:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH1MS_MASK;
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1MS_OUT;
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH1COMCTL_MASK;
		switch (oc_mode) {
		case TIM_COMCTL_FROZEN:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_FROZEN;
			break;
		case TIM_COMCTL_ACTIVE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_ACTIVE;
			break;
		case TIM_COMCTL_INACTIVE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_INACTIVE;
			break;
		case TIM_COMCTL_TOGGLE:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_TOGGLE;
			break;
		case TIM_COMCTL_FORCE_LOW:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_FORCE_LOW;
			break;
		case TIM_COMCTL_FORCE_HIGH:
			TIM_CHCTL0(timer_peripheral) |=
			    TIM_CHCTL0_CH1COMCTL_FORCE_HIGH;
			break;
		case TIM_COMCTL_PWM0:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_PWM0;
			break;
		case TIM_COMCTL_PWM1:
			TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMCTL_PWM1;
			break;
		}
		break;
	case TIM_OC2:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH2MS_MASK;
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2MS_OUT;
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH2COMCTL_MASK;
		switch (oc_mode) {
		case TIM_COMCTL_FROZEN:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_FROZEN;
			break;
		case TIM_COMCTL_ACTIVE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_ACTIVE;
			break;
		case TIM_COMCTL_INACTIVE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_INACTIVE;
			break;
		case TIM_COMCTL_TOGGLE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_TOGGLE;
			break;
		case TIM_COMCTL_FORCE_LOW:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_FORCE_LOW;
			break;
		case TIM_COMCTL_FORCE_HIGH:
			TIM_CHCTL1(timer_peripheral) |=
			    TIM_CHCTL1_CH2COMCTL_FORCE_HIGH;
			break;
		case TIM_COMCTL_PWM0:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_PWM0;
			break;
		case TIM_COMCTL_PWM1:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMCTL_PWM1;
			break;
		}
		break;
	case TIM_OC3:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH3MS_MASK;
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3MS_OUT;
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH3COMCTL_MASK;
		switch (oc_mode) {
		case TIM_COMCTL_FROZEN:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_FROZEN;
			break;
		case TIM_COMCTL_ACTIVE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_ACTIVE;
			break;
		case TIM_COMCTL_INACTIVE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_INACTIVE;
			break;
		case TIM_COMCTL_TOGGLE:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_TOGGLE;
			break;
		case TIM_COMCTL_FORCE_LOW:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_FORCE_LOW;
			break;
		case TIM_COMCTL_FORCE_HIGH:
			TIM_CHCTL1(timer_peripheral) |=
			    TIM_CHCTL1_CH3COMCTL_FORCE_HIGH;
			break;
		case TIM_COMCTL_PWM0:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_PWM0;
			break;
		case TIM_COMCTL_PWM1:
			TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMCTL_PWM1;
			break;
		}
		break;
	case TIM_OC0N:
...
/*---------------------------------------------------------------------------*/
/** @brief Timer Enable the Output Compare shadow Register
...
void timer_enable_oc_shadow(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH0COMSEN;
		break;
	case TIM_OC1:
		TIM_CHCTL0(timer_peripheral) |= TIM_CHCTL0_CH1COMSEN;
		break;
	case TIM_OC2:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH2COMSEN;
		break;
	case TIM_OC3:
		TIM_CHCTL1(timer_peripheral) |= TIM_CHCTL1_CH3COMSEN;
		break;
	case TIM_OC0N:
	case TIM_OC1N:
	case TIM_OC2N:
...
/*---------------------------------------------------------------------------*/
/** @brief Timer Disable the Output Compare shadow Register
...
void timer_set_oc_polarity_high(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH0P;
		break;
	case TIM_OC1:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH1P;
		break;
	case TIM_OC2:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH2P;
		break;
	case TIM_OC3:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH3P;
		break;
	case TIM_OC0N:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH0NP;
		break;
	case TIM_OC1N:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH1NP;
		break;
	case TIM_OC2N:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CH2NP;
		break;
	}
}
...
void timer_enable_oc_output(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH0EN;
		break;
	case TIM_OC1:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH1EN;
		break;
	case TIM_OC2:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH2EN;
		break;
	case TIM_OC3:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH3EN;
		break;
	case TIM_OC0N:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH0NEN;
		break;
	case TIM_OC1N:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH1NEN;
		break;
	case TIM_OC2N:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CH2NEN;
		break;
	}
}
				 ...
				 enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO0;
		break;
	case TIM_OC0N:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO0N;
		break;
	case TIM_OC1:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO1;
		break;
	case TIM_OC1N:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO1N;
		break;
	case TIM_OC2:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO2;
		break;
	case TIM_OC2N:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO2N;
		break;
	case TIM_OC3:
		TIM_CTL1(timer_peripheral) |= TIM_CTL1_ISO3;
		break;
	}
}
...
This is a convenience function to set the OC shadow register value for loading
			...
			uint32_t value)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CH0CV(timer_peripheral) = value;
		break;
	case TIM_OC1:
		TIM_CH1CV(timer_peripheral) = value;
		break;
	case TIM_OC2:
		TIM_CH2CV(timer_peripheral) = value;
		break;
	case TIM_OC3:
		TIM_CH3CV(timer_peripheral) = value;
		break;
	case TIM_OC0N:
	case TIM_OC1N:
	case TIM_OC2N:
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_POEN;
}
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_OAEN;
}
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_BRKP;
}
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_BRKEN;
}
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_ROS;
}
...
{
	TIM_CCHP(timer_peripheral) |= TIM_CCHP_IOS;
}
...
{
	TIM_CCHP(timer_peripheral) |= lock;
}
...
{
	TIM_CCHP(timer_peripheral) |= deadtime;
}
...
useful to cause shadow registers to be shadowed before the timer is started to
...
{
	TIM_SWEVG(timer_peripheral) |= event;
}
			 ...
			 enum tim_ic_filter flt)
{
	switch (ic) {
	case TIM_IC0:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH0CAPFLT_MASK;
		TIM_CHCTL0(timer_peripheral) |= flt << 4;
		break;
	case TIM_IC1:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH1CAPFLT_MASK;
		TIM_CHCTL0(timer_peripheral) |= flt << 12;
		break;
	case TIM_IC2:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH2CAPFLT_MASK;
		TIM_CHCTL1(timer_peripheral) |= flt << 4;
		break;
	case TIM_IC3:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH3CAPFLT_MASK;
		TIM_CHCTL1(timer_peripheral) |= flt << 12;
		break;
	}
}
			    ...
			    enum tim_ic_psc psc)
{
	switch (ic) {
	case TIM_IC0:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH0CAPPSC_MASK;
		TIM_CHCTL0(timer_peripheral) |= psc << 2;
		break;
	case TIM_IC1:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH1CAPPSC_MASK;
		TIM_CHCTL0(timer_peripheral) |= psc << 10;
		break;
	case TIM_IC2:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH2CAPPSC_MASK;
		TIM_CHCTL1(timer_peripheral) |= psc << 2;
		break;
	case TIM_IC3:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH3CAPPSC_MASK;
		TIM_CHCTL1(timer_peripheral) |= psc << 10;
		break;
	}
}
			...
			enum tim_ic_input in)
{
	...
	case TIM_IC0:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH0MS_MASK;
		TIM_CHCTL0(timer_peripheral) |= in;
		break;
	case TIM_IC1:
		TIM_CHCTL0(timer_peripheral) &= ~TIM_CHCTL0_CH1MS_MASK;
		TIM_CHCTL0(timer_peripheral) |= in << 8;
		break;
	case TIM_IC2:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH2MS_MASK;
		TIM_CHCTL1(timer_peripheral) |= in;
		break;
	case TIM_IC3:
		TIM_CHCTL1(timer_peripheral) &= ~TIM_CHCTL1_CH3MS_MASK;
		TIM_CHCTL1(timer_peripheral) |= in << 8;
		break;
	}
}
...
{
	TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_ETFC_MASK;
	TIM_SMCFG(timer_peripheral) |= flt << 8;
}
...
{
	TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_ETPSC_MASK;
	TIM_SMCFG(timer_peripheral) |= psc << 12;
}
...
{
	if (pol) {
		TIM_SMCFG(timer_peripheral) |= TIM_SMCFG_ETP;
	} else {
		TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_ETP;
	}
}
...
{
	TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_SMC_MASK;
	TIM_SMCFG(timer_peripheral) |= mode;
}
...
{
	TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_TRGS_MASK;
	TIM_SMCFG(timer_peripheral) |= trigger;
}
...
{
	if (state) {
		TIM_SMCFG(timer_peripheral) |= TIM_SMCFG_SMC_ECM1;
	} else {
		TIM_SMCFG(timer_peripheral) &= ~TIM_SMCFG_SMC_ECM1;
	}
}
