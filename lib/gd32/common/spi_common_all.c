...
LSB first.
@code
	spi_init_master(SPI0, SPI_CTL0_BAUDRATE_FPCLK_DIV_4, SPI_CTL0_CPOL_CLK_TO_0_WHEN_IDLE,
			SPI_CTL0_CPHA_CLK_TRANSITION_1, SPI_CTL0_DFF_8BIT, SPI_CTL0_LSBFIRST);
	spi_write(SPI0, 0x55);		// 8-bit write
	spi_write(SPI0, 0xaa88);	// 16-bit write
	reg8 = spi_read(SPI0);		// 8-bit read
	reg16 = spi_read(SPI0);		// 16-bit read
@endcode
...
{	switch (spi_peripheral) {
	...
	case SPI0_BASE:
		rcc_periph_reset_pulse(RST_SPI0);
		break;
#endif
...
{
	SPI_CTL0(spi) |= SPI_CTL0_SPIEN; /* Enable SPI. */
}
...
void spi_disable(uint32_t spi)
{
	...
	reg32 = SPI_CTL0(spi);
	reg32 &= ~(SPI_CTL0_SPIEN); /* Disable SPI. */
	SPI_CTL0(spi) = reg32;
}
...
prevents the TRANS flag from becoming unreliable.
...
uint16_t spi_clean_disable(uint32_t spi)
{
	...
	uint16_t data = SPI_DATA(spi);

	/* Wait to transmit last data */
	while (!(SPI_STAT(spi) & SPI_STAT_TBE));

	/* Wait until not busy */
	while (SPI_STAT(spi) & SPI_STAT_TRANS);
...
@todo these should perhaps be combined with an SWNSSEN enable as it is meaningless
otherwise
...
{
	SPI_CTL0(spi) |= SPI_CTL0_LSBFIRST;
}
...
void spi_set_baudrate_prescaler(uint32_t spi, uint8_t baudrate)
{
	...
	reg32 = (SPI_CTL0(spi) & 0xffc7); /* Clear bits [5:3]. */
	reg32 |= (baudrate << 3);
	SPI_CTL0(spi) = reg32;
}
...
void spi_set_standard_mode(uint32_t spi, uint8_t mode)
{
	...
	uint32_t reg32 = SPI_CTL0(spi) & ~(SPI_CTL0_CPOL | SPI_CTL0_CPHA);
	SPI_CTL0(spi) = reg32 | mode;
}
