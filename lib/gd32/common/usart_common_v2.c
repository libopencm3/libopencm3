...
@ingroup peripheral_apis
 ...
 @author @htmlonly &copy; @endhtmlonly 2022 Cem Basoglu <cem.basoglu@web.de>
 ...
 *
 * Copyright (C) 2022 Cem Basoglu <cem.basoglu@web.de>
 *
...
{
	USART_CTL1(usart) |= USART_CTL1_DINV;
}
...
{
	USART_CTL1(usart) |= USART_CTL1_TINV;
}
...
{
	USART_CTL1(usart) |= USART_CTL1_RINV;
}
...
{
	USART_CTL2(usart) |= USART_CTL2_HDEN;
}
...
{
	uint32_t reg;
	reg = USART_RT(usart) & ~USART_RT_RT_MASK;
	reg |= (USART_RT_RT_VAL(value) & USART_RT_RT_MASK);
	USART_RT(usart) = reg;
}
...
{
	USART_CTL1(usart) |= USART_CTL1_RTEN;
}
...
{
	USART_CTL0(usart) |= USART_CTL0_RTIE;
}
...
{
	/* Send data. */
	USART_TDATA(usart) = (data & USART_TDATA_MASK);
}
...
{
	/* Receive data. */
	return USART_RDATA(usart) & USART_RDATA_MASK;
}
...
void usart_wait_send_ready(uint32_t usart)
{
	...
	while ((USART_STAT(usart) & USART_STAT_TBE) == 0);
}
...
void usart_wait_recv_ready(uint32_t usart)
{
	...
	while ((USART_STAT(usart) & USART_STAT_RBNE) == 0);
}
