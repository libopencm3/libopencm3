...
static inline void ltdc_layer_ctrl_disable(uint32_t layer,
                                           uint32_t ctrl_flags) {
  LTDC_LxCR(layer) &= ~(ctrl_flags);
}

static inline void ltdc_set_color_key(uint32_t layer, uint8_t r, uint8_t g,
                                      uint8_t b) {
  LTDC_LxCKCR(layer) =
...
static inline void ltdc_set_default_colors(uint32_t layer, uint8_t a, uint8_t r,
                                           uint8_t g, uint8_t b) {
  LTDC_LxDCCR(layer) = ((((a)&255) << 24) | (((r)&255) << 16) |
                        (((g)&255) << 8) | (((b)&255) << 0));
}

static inline void ltdc_set_blending_factors(uint32_t layer, uint8_t bf1,
                                             uint8_t bf2) {
...
static inline void ltdc_set_fb_line_length(uint32_t layer, uint16_t len,
                                           uint16_t pitch) {
...
static inline bool LTDC_SRCR_IS_RELOADING(void) {
  return (LTDC_SRCR & (LTDC_SRCR_RELOAD_VBR | LTDC_SRCR_RELOAD_IMR)) != 0;
}
